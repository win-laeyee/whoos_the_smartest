...
def collate_document_data(query):
    docs = query.stream()
    documents = []
    for doc in docs:
...
def get_all_docs(db, user_id, coll_name):
    """
...
def update_doc_with_timestamp(db: firestore.Client, user_id: str, coll_name: str) -> None:
    """
...
def delete_all_docs_in_collection(db: firestore.Client, coll_name: str, batch_size: int, user_id: Optional[str] = None) -> None:
    """
